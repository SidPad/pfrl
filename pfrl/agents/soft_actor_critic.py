...
class MTSoftActorCritic(AttributeSavingMixin, BatchAgent):
    ...
    def temperature(self):
        if self.entropy_target is None:
            ...
            with torch.no_grad():
                return float(self.temperature_holder1()), float(self.temperature_holder3()) #, float(self.temperature_holder2())
    ...
    def update_temperature(self, log_prob1, log_prob3): # log_prob2, 
    ...
    def update_policy_and_temperature(self, batch):
        ...
        loss = (loss_T1 + loss_T3) / N # loss_T2 + 
        ...
        if self.entropy_target is not None:
            self.update_temperature(log_prob1.detach(), log_prob3.detach()) # log_prob2.detach(), 
