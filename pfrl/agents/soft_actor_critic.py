...
class MTSoftActorCritic(AttributeSavingMixin, BatchAgent):
    ...
    def update_q_func(self, batch):        
        ...
        last_batch_next_state = batch_next_state[(self.seq_len - 1)::self.seq_len]        
        ...
        last_batch_state = batch_state[(self.seq_len - 1)::self.seq_len]        
        ...
        with torch.no_grad(), pfrl.utils.evaluating(self.policy1), pfrl.utils.evaluating(
            ...
            self.shared_q_critic), pfrl.utils.evaluating(self.shared_layer_critic):               
                ...
                # with torch.cuda.amp.autocast():
                next_action_distrib1 = self.policy1(batch_input_next_state_critic1)
        ...
        # with torch.cuda.amp.autocast():
        predict_q1_T1 = torch.flatten(self.q_func1_T1((last_batch_state, last_action)))
        predict_q2_T1 = torch.flatten(self.q_func2_T1((last_batch_state, last_action)))
        ...
        # Update stats
        if batch_input_next_state_critic1.numel() > 0:
        ...
        loss1_T1.backward(retain_graph=True)
    ...
    def update_policy_and_temperature(self, batch):        
        ...
        # actions = actions[(self.seq_len - 1)::self.seq_len]
            ...
            q1_T1 = self.q_func1_T1((last_batch_state, actions1))
            q2_T1 = self.q_func2_T1((last_batch_state, actions1))
