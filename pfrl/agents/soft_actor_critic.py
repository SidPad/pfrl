...
class SoftActorCritic(AttributeSavingMixin, BatchAgent):
    ...
    def update_q_func(self, batch):
        ...
        stddevs = torch.abs(batch_next_state_red * batch_next_state[:, -1].unsqueeze(1))
    ...
    def update_policy_and_temperature(self, batch):
        ...
        stddevs = torch.abs(batch_state_red * batch_state[:, -1].unsqueeze(1))
    ...
    def batch_select_greedy_action(self, batch_obs, deterministic=False):
        with torch.no_grad(), pfrl.utils.evaluating(self.policy):
            ...
            print(batch_xs)
            print(batch_xs[:, -1])
            stddevs = torch.abs(batch_xs_red * batch_xs[0, -1].unsqueeze(1))
...
class MTSoftActorCritic(AttributeSavingMixin, BatchAgent):
    ...
    def update_q_func(self, batch):
        ...
        with torch.autograd.profiler.profile(use_cuda=True) as prof:
            ...
            for j in range(len(ep_len_actual_sum2)): 
                if j in indicesA:
                    random_indexA = random.randint(ep_len_actual_sum2[j], ep_len_actual_sum1[j] - 30)
                    indicesAA = np.append(indicesAA, [i for i in range(random_indexA, random_indexA + 30)])
                if j in indicesB:
            ...
            batch_next_state = batch_next_state[self.indices]

            batch_state = torch.cat(batch_state)
            batch_state = batch_state[self.indices]

            batch_actions = batch_actions[self.indices]

            batch_next_state = nn.utils.rnn.pad_sequence(batch_next_state, batch_first=True, padding_value=0)
            if len(batch_next_state) < 300:
                batch_next_state = torch.cat([batch_next_state, torch.zeros(300-len(batch_next_state), batch_next_state.shape[0]).to(self.device)], dim=1)
            batch_next_state = torch.split(batch_next_state, 30, dim=0)
            ...
            if len(batch_state) < 300:
                batch_state = torch.cat([batch_state, torch.zeros(300-len(batch_state), batch_state.shape[0]).to(self.device)], dim=1)
            batch_state = torch.split(batch_state, 30, dim=0)
            ...
            if len(batch_actions) < 300:
                batch_actions = torch.cat([batch_actions, torch.zeros(300-len(batch_actions), batch_actions.shape[0]).to(self.device)], dim=1)
            batch_actions = torch.split(batch_actions, 30, dim=0)
            ...
            batch_input_next_state = [torch.cat((batch_next_state, batch_actions), dim = 1).to(torch.float32) for batch_next_state, batch_actions in zip(batch_next_state, batch_actions)]
            
            batch_rewards_clone = batch_rewards.clone().detach()
            batch_rewards1 = batch_rewards_clone[self.indicesAA]
            batch_rewards1 = torch.cat((batch_rewards1, torch.zeros(300 - len(batch_rewards1)).to(self.device)), dim=0)
            ...
            batch_rewards2 = torch.cat((batch_rewards2, torch.zeros(300 - len(batch_rewards2)).to(self.device)), dim=0)
            ...
            batch_rewards3 = torch.cat((batch_rewards3, torch.zeros(300 - len(batch_rewards3)).to(self.device)), dim=0)
            ...
            batch_discount_clone = batch_discount.clone().detach()
            batch_discount1 = batch_discount_clone[self.indicesAA]
            batch_discount1 = torch.cat((batch_discount1, torch.zeros(300 - len(batch_discount1)).to(self.device)), dim=0)
            batch_discount2 = batch_discount_clone[self.indicesBB]
            batch_discount2 = torch.cat((batch_discount2, torch.zeros(300 - len(batch_discount2)).to(self.device)), dim=0)
            batch_discount3 = batch_discount_clone[self.indicesCC]
            batch_discount3 = torch.cat((batch_discount3, torch.zeros(300 - len(batch_discount3)).to(self.device)), dim=0)

            batch_terminal_clone = batch_terminal.clone().detach()
            batch_terminal1 = batch_terminal_clone[self.indicesAA]
            batch_terminal1 = torch.cat((batch_terminal1, torch.zeros(300 - len(batch_terminal1)).to(self.device)), dim=0)
            batch_terminal2 = batch_terminal_clone[self.indicesBB]
            batch_terminal2 = torch.cat((batch_terminal2, torch.zeros(300 - len(batch_terminal2)).to(self.device)), dim=0)
            batch_terminal3 = batch_terminal_clone[self.indicesCC]
            batch_terminal3 = torch.cat((batch_terminal3, torch.zeros(300 - len(batch_terminal3)).to(self.device)), dim=0)

            batch_actions_clone = [batch_actions for batch_actions in batch_actions]
            batch_actions_clone = torch.cat(batch_actions_clone)
            ...
            batch_actions1 = torch.cat((batch_actions1, torch.zeros(300-len(self.indicesAA), 23).to(self.device))).to(torch.float32)
            batch_actions2 = torch.cat((batch_actions2, torch.zeros(300-len(self.indicesBB), 23).to(self.device))).to(torch.float32)
            batch_actions3 = torch.cat((batch_actions3, torch.zeros(300-len(self.indicesCC), 23).to(self.device))).to(torch.float32)
            
            #### TASK 1 #### Figure out what pfrl.utils.evaluating does
            with torch.no_grad(), pfrl.utils.evaluating(self.policy1), pfrl.utils.evaluating(self.policy2), pfrl.utils.evaluating(self.policy3), pfrl.utils.evaluating(
                self.target_q_func1_T1
            ), pfrl.utils.evaluating(self.target_q_func2_T1), pfrl.utils.evaluating(
                self.target_q_func1_T2
            ), pfrl.utils.evaluating(self.target_q_func2_T2), pfrl.utils.evaluating(
                self.target_q_func1_T3
            ), pfrl.utils.evaluating(self.target_q_func2_T3), pfrl.utils.evaluating(
                ...
                batch_input_next_state_critic, _ = pack_and_forward(self.shared_q_critic, batch_input_next_state, batch_next_recurrent_state_critic)
                batch_input_next_state_critic = self.shared_layer_critic(batch_input_next_state_critic)

                batch_input_next_state_actor, _ = pack_and_forward(self.shared_q_actor, batch_input_next_state, batch_next_recurrent_state_actor)
                batch_input_next_state_actor = self.shared_layer_actor(batch_input_next_state_actor)
                
                batch_input_state, _ = pack_and_forward(self.shared_q_critic, batch_input_state, batch_recurrent_state_critic)
                batch_input_state = self.shared_layer_critic(batch_input_state)
                ...
                batch_input_next_state_critic1 = torch.cat((batch_input_next_state_critic1, torch.zeros(300-len(self.indicesAA), 61).to(self.device))).to(torch.float32)
                batch_input_next_state_critic2 = torch.cat((batch_input_next_state_critic2, torch.zeros(300-len(self.indicesBB), 61).to(self.device))).to(torch.float32)
                batch_input_next_state_critic3 = torch.cat((batch_input_next_state_critic3, torch.zeros(300-len(self.indicesCC), 61).to(self.device))).to(torch.float32)
                ...
                batch_input_next_state_actor1 = torch.cat((batch_input_next_state_actor1, torch.zeros(300-len(self.indicesAA), 61).to(self.device))).to(torch.float32)
                batch_input_next_state_actor2 = torch.cat((batch_input_next_state_actor2, torch.zeros(300-len(self.indicesBB), 61).to(self.device))).to(torch.float32)
                batch_input_next_state_actor3 = torch.cat((batch_input_next_state_actor3, torch.zeros(300-len(self.indicesCC), 61).to(self.device))).to(torch.float32)
                ...
                batch_input_state1 = torch.cat((batch_input_state1, torch.zeros(300-len(self.indicesAA), 61).to(self.device))).to(torch.float32)
                batch_input_state2 = torch.cat((batch_input_state2, torch.zeros(300-len(self.indicesBB), 61).to(self.device))).to(torch.float32)
                batch_input_state3 = torch.cat((batch_input_state3, torch.zeros(300-len(self.indicesCC), 61).to(self.device))).to(torch.float32)
    ...
    def update_policy_and_temperature(self, batch):
        ...
        batch_input_state_critic, _ = pack_and_forward(self.shared_q_critic, batch_input_state, batch_recurrent_state_critic)
        batch_input_state_critic = self.shared_layer_critic(batch_input_state_critic)

        batch_input_state_actor, _ = pack_and_forward(self.shared_q_actor, batch_input_state, batch_recurrent_state_actor)
        batch_input_state_actor = self.shared_layer_actor(batch_input_state_actor)
        ...
        batch_input_state_critic1 = torch.cat((batch_input_state_critic1, torch.zeros(300-len(self.indicesAA), 61).to(self.device))).to(torch.float32)
        batch_input_state_critic2 = torch.cat((batch_input_state_critic2, torch.zeros(300-len(self.indicesBB), 61).to(self.device))).to(torch.float32)
        batch_input_state_critic3 = torch.cat((batch_input_state_critic3, torch.zeros(300-len(self.indicesCC), 61).to(self.device))).to(torch.float32)
        ...
        batch_input_state_actor1 = torch.cat((batch_input_state_actor1, torch.zeros(300-len(self.indicesAA), 61).to(self.device))).to(torch.float32)
        batch_input_state_actor2 = torch.cat((batch_input_state_actor2, torch.zeros(300-len(self.indicesBB), 61).to(self.device))).to(torch.float32)
        batch_input_state_actor3 = torch.cat((batch_input_state_actor3, torch.zeros(300-len(self.indicesCC), 61).to(self.device))).to(torch.float32)
        ...
        with torch.no_grad(), pfrl.utils.evaluating(self.policy1), pfrl.utils.evaluating(self.policy2), pfrl.utils.evaluating(self.policy3):
            ...
            if self.recurrent:
                if self.training:
                    ...
                    batch_input_actor = self.shared_layer_actor(batch_input_actor)
                else:
            ...
            policy_out1 = self.policy1(batch_xs1)
